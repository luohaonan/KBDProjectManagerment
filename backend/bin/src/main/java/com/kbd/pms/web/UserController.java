...
public class UserController {
    ...
    public ResponseEntity<Result<UserDto>> createUser(@RequestBody CreateUserRequest request) {
        ...
        if (request.getDepartmentIds() != null && !request.getDepartmentIds().isEmpty()) {
            Set<OrgDepartmentEntity> depts = new HashSet<>();
    ...
    public ResponseEntity<Result<CurrentUserInfo>> getCurrentUser(Authentication authentication) {
        ...
        if (depts != null && !depts.isEmpty()) {
            info.setDepartmentIds(depts.stream().map(OrgDepartmentEntity::getId).collect(Collectors.toList()));
            info.setDepartmentNames(depts.stream().map(OrgDepartmentEntity::getDeptName).collect(Collectors.toList()));
        }
